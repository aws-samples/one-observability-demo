...
export class Services extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const s3_observabilitypetadoptions = new s3.Bucket(
      this,
      "s3bucket_petadoption",
      {
    ...
    new s3seeder.BucketDeployment(this, "s3seeder_petadoption", {
      ...
      sources: [
        s3seeder.Source.asset("./resources/kitten.zip"),
        s3seeder.Source.asset("./resources/puppies.zip"),
        s3seeder.Source.asset("./resources/bunnies.zip"),
      ],
    });
    ...
    rdssecuritygroup.addIngressRule(
      ...
      ec2.Port.tcp(5432),
      "Allow Aurora PG access from within the VPC CIDR range"
    );
    ...
    const ecsServicesSecurityGroup = new ec2.SecurityGroup(
      this,
      "ECSServicesSG",
      {
        vpc: theVPC,
    ...
    ecsServicesSecurityGroup.addIngressRule(
      ec2.Peer.ipv4(theVPC.vpcCidrBlock),
      ec2.Port.tcp(80)
    );
    ...
    payForAdoptionService.taskDefinition.taskRole?.addToPrincipalPolicy(
      readSSMParamsPolicy
    );
    payForAdoptionService.taskDefinition.taskRole?.addToPrincipalPolicy(
      ddbSeedPolicy
    ...
    const ecsPetListAdoptionCluster = new ecs.Cluster(
      this,
      "PetListAdoptions",
      {
        vpc: theVPC,
    ...
    listAdoptionsService.taskDefinition.taskRole?.addToPrincipalPolicy(
      readSSMParamsPolicy
    );
    ...
    const trafficGeneratorService = new TrafficGeneratorService(
      this,
    ...
    trafficGeneratorService.taskDefinition.taskRole?.addToPrincipalPolicy(
      readSSMParamsPolicy
    );

    //PetStatusUpdater Lambda Function and APIGW--------------------------------------
    const statusUpdaterService = new StatusUpdaterService(
      this,
    ...
    const petadoptionshistory_targetGroup = new elbv2.ApplicationTargetGroup(
      this,
    ...
    var ssmAgentSetup = yaml.loadAll(
      readFileSync("./resources/setup-ssm-agent.yaml", "utf8")
    ) as Record<string, any>[];

    const ssmAgentSetupManifest = new eks.KubernetesManifest(
      this,
      "ssmAgentdeployment",
      {
        cluster: cluster,
    ...
    ); // Remove https:// from the URL as workaround to get ClusterID
    ...
    const cw_federatedPrincipal = new iam.FederatedPrincipal(
      cluster.openIdConnectProvider.openIdConnectProviderArn,
      {
        StringEquals: new CfnJson(this, "CW_FederatedPrincipalCondition", {
          value: {
            [`oidc.eks.${region}.amazonaws.com/id/${clusterId}:aud`]:
              "sts.amazonaws.com",
          },
        }),
      }
    ...
    const cwserviceaccount = new iam.Role(this, "CWServiceAccount", {
      ...
      managedPolicies: [
        iam.ManagedPolicy.fromManagedPolicyArn(
          this,
          "CWServiceAccount-CloudWatchAgentServerPolicy",
    ...
    const xrayserviceaccount = new iam.Role(this, "XRayServiceAccount", {
      ...
      managedPolicies: [
        iam.ManagedPolicy.fromManagedPolicyArn(
          this,
          "XRayServiceAccount-AWSXRayDaemonWriteAccess",
    ...
    const loadBalancerPolicyDoc = iam.PolicyDocument.fromJson(
      JSON.parse(
        readFileSync("./resources/load_balancer/iam_policy.json", "utf8")
      )
    );
    const loadBalancerPolicy = new iam.ManagedPolicy(
      this,
      "LoadBalancerSAPolicy",
      { document: loadBalancerPolicyDoc }
    );
    const loadBalancerserviceaccount = new iam.Role(
      this,
      "LoadBalancerServiceAccount",
      {
        //                assumedBy: eksFederatedPrincipal,
        assumedBy: new iam.AccountRootPrincipal(),
    ...
    loadBalancerserviceaccount.assumeRolePolicy?.addStatements(
      loadBalancer_trustRelationship
    );
    ...
    var xRayYaml = yaml.loadAll(
      readFileSync("./resources/k8s_petsite/xray-daemon-config.yaml", "utf8")
    ) as Record<string, any>[];

    xRayYaml[0].metadata.annotations["eks.amazonaws.com/role-arn"] =
      new CfnJson(this, "xray_Role", {
        value: `${xrayserviceaccount.roleArn}`,
      });
    ...
    var loadBalancerServiceAccountYaml = yaml.loadAll(
      readFileSync("./resources/load_balancer/service_account.yaml", "utf8")
    ) as Record<string, any>[];
    loadBalancerServiceAccountYaml[0].metadata.annotations[
      "eks.amazonaws.com/role-arn"
    ] = new CfnJson(this, "loadBalancer_Role", {
      value: `${loadBalancerserviceaccount.roleArn}`,
    });

    const loadBalancerServiceAccount = new eks.KubernetesManifest(
      this,
      "loadBalancerServiceAccount",
      {
        cluster: cluster,
    ...
    const waitForLBServiceAccount = new eks.KubernetesObjectValue(
      this,
      "LBServiceAccount",
      {
        cluster: cluster,
    ...
    const loadBalancerCRDYaml = yaml.loadAll(
      readFileSync("./resources/load_balancer/crds.yaml", "utf8")
    ) as Record<string, any>[];
    const loadBalancerCRDManifest = new eks.KubernetesManifest(
      this,
      "loadBalancerCRD",
      {
        cluster: cluster,
    ...
    const awsLoadBalancerManifest = new eks.HelmChart(
      this,
    ...
    customWidgetLambdaRole.addToPrincipalPolicy(
      customWidgetResourceControllerPolicy
    );

    var petsiteApplicationResourceController = new lambda.Function(
      this,
      "petsite-application-resource-controler",
      {
        code: lambda.Code.fromAsset(
          path.join(__dirname, "/../resources/resource-controller-widget")
        ),
    ...
    petsiteApplicationResourceController.addEnvironment(
      "EKS_CLUSTER_NAME",
      cluster.clusterName
    );
    petsiteApplicationResourceController.addEnvironment(
      "ECS_CLUSTER_ARNS",
      ecsPayForAdoptionCluster.clusterArn +
        "," +
        ecsPetListAdoptionCluster.clusterArn +
        "," +
        ecsPetSearchCluster.clusterArn
    );

    var customWidgetFunction = new lambda.Function(
      this,
    ...
    customWidgetFunction.addEnvironment(
      "CONTROLER_LAMBDA_ARN",
      petsiteApplicationResourceController.functionArn
    );
    customWidgetFunction.addEnvironment(
      "EKS_CLUSTER_NAME",
      cluster.clusterName
    ...
    var costControlDashboardBody = readFileSync(
      "./resources/cw_dashboard_cost_control.json",
      "utf-8"
    );
    costControlDashboardBody = costControlDashboardBody.replaceAll(
      "{{YOUR_LAMBDA_ARN}}",
      customWidgetFunction.functionArn
    );

    const petSiteCostControlDashboard = new cloudwatch.CfnDashboard(
      this,
      "PetSiteCostControlDashboard",
      {
    ...
    const servicesCfnGroup = new resourcegroups.CfnGroup(
      this,
      "ServicesCfnGroup",
      {
        name: stackName,
        description:
          "Contains all the resources deployed by Cloudformation Stack " +
          stackName,
        resourceQuery: {
    ...
    const servicesCfnApplication = new applicationinsights.CfnApplication(
      this,
    ...
    var dynamodbQueryLambdaRole = new iam.Role(
      this,
      "dynamodbQueryLambdaRole",
      {
        assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com"),
        managedPolicies: [
          iam.ManagedPolicy.fromManagedPolicyArn(
    ...
    var dynamodbQueryFunction = new lambda.Function(
      this,
      "dynamodb-query-function",
      {
        code: lambda.Code.fromAsset(
          path.join(__dirname, "/../resources/application-insights")
        ),
    ...
    dynamodbQueryFunction.addEnvironment(
      "DYNAMODB_TABLE_NAME",
      dynamodb_petadoption.tableName
    );
    ...
    this.createSsmParameters(
      new Map(
        Object.entries({
          ...
          "/petstore/rumscript": " ",
          "/petstore/petadoptionsstepfnarn":
            petAdoptionsStepFn.stepFn.stateMachineArn,
          ...
          "/petstore/petsearch-collector-manual-config": readFileSync(
            "./resources/collector/ecs-xray-manual.yaml",
            "utf8"
          ),
