...
# Prometheus metrics
REQUEST_COUNT = Counter('petlistadoptions_requests_total', 'Number of requests received', ['endpoint', 'error'])
REQUEST_LATENCY = Histogram('petlistadoptions_requests_latency_seconds', 'Request durations in seconds', ['endpoint', 'error'])
...
class DatabaseConfig:
    ...
    def __init__(self, engine: str, host: str, username: str, password: str, dbname: str, port: int):
        self.engine = engine
        self.host = host
...
class PetAdoptionsService:
    ...
    def _get_database_connection_string(self) -> str:
        ...
        try:
            ...
            if self.rds_secret_arn == "local-secret":
                # Read from local file for testing
                with open("/app/local-secret.json", "r") as f:
            ...
            connection_string = f"postgresql://{secret_data['username']}:{secret_data['password']}@{secret_data['host']}:{secret_data.get('port', 5432)}/{secret_data['dbname']}"
    ...
    def _get_latest_adoptions(self) -> List[Dict[str, Any]]:
        ...
        with self._get_database_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT pet_id, transaction_id, adoption_date FROM transactions ORDER BY id DESC LIMIT 25")
    ...
    def list_adoptions(self) -> List[Adoption]:
        ...
        try:
            ...
            REQUEST_LATENCY.labels(endpoint='adoptionlist', error='false').observe(duration)
            ...
            REQUEST_LATENCY.labels(endpoint='adoptionlist', error='true').observe(duration)
