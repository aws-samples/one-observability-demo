#!/bin/bash
# Deadlock Simulator (Debug Version) - Same as deadlock-simulator.sh but with verbose output for debugging.
# Shows detailed execution steps, timing information, and progress updates during deadlock simulation.
...
psql -c "DROP TABLE IF EXISTS CustomerOrders;"
psql -c "CREATE TABLE CustomerOrders (id SERIAL PRIMARY KEY, value INTEGER);"
psql -c "INSERT INTO CustomerOrders (value) SELECT generate_series(1, 10);"

echo "Test table created with 10 rows"
echo ""
...
run_transaction() {
    ...
    psql -c "BEGIN;
             ...
             COMMIT;" 2>&1 | tee /tmp/session${session_id}.log
    ...
    if grep -qi "deadlock" /tmp/session${session_id}.log; then
        ...
        # Count current deadlocks for display
        CURRENT_COUNT=$(wc -l < /tmp/deadlock_counter.txt 2>/dev/null || echo "0")
        echo "!!! Deadlock #$CURRENT_COUNT: DETECTED in Cycle $cycle_num, Session $session_id !!!"
    fi
}
...
psql -c "DROP TABLE IF EXISTS CustomerOrders;"
echo "  ✓ CustomerOrders table dropped"
