...
def generate_prompt(food_data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        prompt_builder = EnhancedPromptBuilder(
            food_name, pet_type, food_type, description, 
            ingredients, nutritional_info, feeding_guidelines, price
        )
...
class EnhancedPromptBuilder:
    """Advanced prompt builder with contextual intelligence and visual sophistication."""
    
    def __init__(self, food_name: str, pet_type: str, food_type: str, 
                 description: str, ingredients: list, nutritional_info: dict, 
                 feeding_guidelines: str, price: float):
        ...
        self.feeding_guidelines = feeding_guidelines.lower() if feeding_guidelines else ''
        ...
        self.premium_indicators = [
            'premium', 'gourmet', 'organic', 'natural', 'grain-free', 
            'free-range', 'wild-caught', 'artisan', 'holistic'
        ]
    ...
    def build(self) -> str:
        ...
        food_characteristics = self._build_food_characteristics(pet_context, food_context)
    ...
    def _detect_premium_product(self) -> bool:
        ...
        has_premium_keywords = any(keyword in text for keyword in self.premium_indicators)
        
        # Check price point with more nuanced thresholds
        price_threshold = {
            'puppy': 25.0, 'kitten': 20.0, 'bunny': 20.0, 'rabbit': 20.0,
            'dog': 30.0, 'cat': 25.0
        }
        ...
        premium_ingredients = ['salmon', 'tuna', 'lamb', 'duck', 'venison', 'bison', 'truffle']
        has_premium_ingredients = any(ing in " ".join(self.ingredients) for ing in premium_ingredients)
        ...
        return (has_premium_keywords or is_high_price or has_premium_ingredients) and not has_budget_keywords
    
    def _get_styling_context(self, is_premium: bool, pet_context: dict, food_context: dict) -> str:
...
def _intelligent_truncate(prompt: str, max_length: int) -> str:
    ...
    essential_keywords = [
        "premium pet food", "bowl", "featuring", "chunks", "pieces", 
        "kibble", "meat", "fish", "chicken", "beef", "salmon"
    ]
    
    important_keywords = [
        "photography", "lighting", "presentation", "professional", 
    ...
    for sentence in essential_sentences:
        test_length = current_length + len(sentence) + (2 if result_parts else 0)  # +2 for ". "
...
def generate_image_with_bedrock(prompt: str, food_id: str) -> Dict[str, Any]:
    ...
    logger.info(f"Adding initial delay of {initial_delay:.2f}s to avoid concurrency issues")
    time.sleep(initial_delay)
    
    for attempt in range(MAX_RETRIES + 1):
        try:
            logger.info(f"Bedrock generation attempt {attempt + 1}/{MAX_RETRIES + 1} for food {food_id}")
            ...
            if 'images' in response_body and len(response_body['images']) > 0:
                ...
                logger.info(f"✅ Successfully generated image for food {food_id} on attempt {attempt + 1}")
                return {
            ...
            error_msg = str(e)
            logger.warning(f"❌ Bedrock attempt {attempt + 1} failed for food {food_id}: {error_msg}")
            
            # Check if this is the last attempt
            if attempt == MAX_RETRIES:
                logger.error(f"🚫 All {MAX_RETRIES + 1} attempts failed for food {food_id}")
                return {
                    "image_data": None,
                    "success": False,
            ...
            logger.info(f"⏳ Retrying in {delay:.2f}s (attempt {attempt + 2}/{MAX_RETRIES + 1})")
            time.sleep(delay)
