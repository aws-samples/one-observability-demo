...
def lambda_handler(event, context):
    ...
    if not table_name:
        ...
        try:
            ...
            for table in all_tables:
                try:
                    table_arn = dynamodb_client.describe_table(TableName=table)[
                        "Table"
                    ]["TableArn"]
            ...
            if not matching_tables:
                ...
                return {
                    "statusCode": 404,
                    "body": json.dumps(
                        {"error": f"No tables found with tag {tag_key}={tag_value}"},
                    ),
                }
    ...
    def create_sized_item(wcu_size, pet_id):
        ...
        if padding_size > 0:
            padding = "".join(
                random.choice(string.ascii_letters) for _ in range(padding_size)
            )
            item["data"] = padding
    ...
    for i in range(iterations):
        try:
            ...
            consumed_wcu = response.get("ConsumedCapacity", {}).get(
                "CapacityUnits",
                "unknown",
            )
    ...
    total_wcu = sum(
        r.get("consumed_wcu", 0)
        for r in successful_writes
        if isinstance(r.get("consumed_wcu"), (int, float))
    )
