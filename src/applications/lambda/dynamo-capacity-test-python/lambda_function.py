...
def lambda_handler(event, context):
    ...
    logger.info(f"Lambda invoked with parameters: tag_key={tag_key}, tag_value={tag_value}, table_pattern={table_pattern}, iterations={iterations}")
        ...
        try:
            ...
            if table_pattern and len(matching_tables) > 1:
                ...
                if filtered_tables:
                    ...
                    logger.info(f"Filtered to tables matching pattern '{table_pattern}': {matching_tables}")
                else:
                    logger.error(f"No tables match pattern '{table_pattern}' in {matching_tables}")
                    return {
                        'statusCode': 404,
                        'body': json.dumps({
