#!/bin/bash
# A/B Test Traffic Generator for Config Bundle Gateway
# Usage: bash loadgen-ab-test-config-bundle.sh [count]

REGION="${AWS_REGION:-us-east-1}"
PROFILE="${AWS_PROFILE:-default}"
TARGET="petfood-agent"
...
GATEWAY_ID=$(aws bedrock-agentcore-control list-gateways --profile "$PROFILE" --region "$REGION" --output json 2>/dev/null \
  | python3 -c "import sys,json;data=json.load(sys.stdin);gws=data.get('items',data.get('gateways',[]));print(next((g['gatewayId'] for g in gws if 'ConfigBundle' in g['name']),''))" 2>/dev/null)

if [ -z "$GATEWAY_ID" ]; then
  echo "ERROR: Could not find PetFoodConfigBundleGateway. Run setup-ab-test-config-bundle.py first."
  exit 1
fi
...
echo "=== Config Bundle A/B Test Traffic Generator ==="
