...
"""
This function will be triggered by Cloudwatch Custom Widget lambda function.
You can also trigger resource-controller function manually with below event.
...
def set_ecs_desired_task_count_to_normal(cluster):
    """
    Function to set ECS Desired Tasks counts to 2 for all ECS Cluster
    Services except trafficgeneratorservice
    """
    services = ecs_client.list_services(cluster=cluster)
    for service in services["serviceArns"]:
        desired_count = 2
